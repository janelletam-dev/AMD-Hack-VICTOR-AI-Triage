...
export default function ClinicianDashboard() {
  ...
  // HEART score for chest-pain cases — clinical (H+A+R, max 6/10).
  // Backend emits `risk_score` events with the full breakdown; we
  // surface a compact badge in IdentityCard near Suspected diagnosis.
  ...
  const processEvent = useCallback((type, data, ts) => {
    if (type === "transcript") {
      ...
      // Clinical risk-strat number from clinical_knowledge.py. Today
      // only HEART (chest pain) is wired; other scores would land here
      // too (Wells/PE, Alvarado/appy, NIHSS/stroke). Stored verbatim
      // so the dashboard can render the breakdown when needed.
      setRiskScore(data);
      if (ts) setLogs(prev => [...prev.slice(-50), {
        ts, color: "#ffb46f",
        text: `RISK: ${data?.score || "?"} clinical = ${data?.clinical_total ?? "?"}/${data?.max_clinical ?? "?"} — ${data?.interpretation || ""}`,
      }]);
...
function IdentityCard({ identity, flagQueue, transcript, riskScore }) {
  ...
  return (
    <section className="vic-glass" style={{
      ...
      {/* Clinical risk score — only renders when backend has computed
          one (e.g. HEART for chest-pain cases). The clinical_total
          here is partial (max 6) since EKG + troponin land at the
          bedside; the badge labels that explicitly so a clinician
          doesn't read it as a final HEART tier. */}
      {riskScore && !editing && (
        <RiskScoreBadge data={riskScore} />
      )}
...
function RiskScoreBadge({ data }) {
  ...
  const max = data.max_clinical ?? 6;
  // Color: green (low pre-test) → amber (intermediate) → red (concerning).
  // Thresholds match clinical_knowledge._heart_interpretation().
  const tone = total >= 4 ? "danger" : total >= 2 ? "warning" : "ok";
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span title={h.justification || ""}>
          H={h.points ?? "-"}
        </span>
        <span style={{ opacity: 0.4 }}>·</span>
        <span title={a.value != null ? `Age ${a.value}` : "Age unknown"}>
          A={a.points ?? "-"}{a.value != null ? ` (${a.value}y)` : ""}
        </span>
        <span style={{ opacity: 0.4 }}>·</span>
        <span title={(r.found || []).join(", ") || "No risk factors detected"}>
          R={r.points ?? "-"}
          {r.found && r.found.length > 0
            ? ` (${r.found.length})`
            : ""}
        </span>
        <span style={{ opacity: 0.4 }}>·</span>
        <span style={{ opacity: 0.6 }}>E,T pending</span>
      </div>
    </div>
  );
}
